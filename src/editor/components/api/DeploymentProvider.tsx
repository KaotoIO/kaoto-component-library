...
function DeploymentProvider({ initialState, children }: IDeploymentProvider) {
  const [deployment, setDeployment] = useState<IDeployment>(initialState ?? initialOrChangedDeployment);

  return <deploymentContext.Provider value={[deployment, setDeployment]}>{children}</deploymentContext.Provider>;
}
