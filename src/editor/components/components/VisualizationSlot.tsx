  ...
  return (
    ...
    >
      {!(data.connectorType === "START") && (
        <Handle type="target" position={Position.Left} id="a" style={{ borderRadius: 0 }} isConnectable={false} />
      )}
      <div className={"stepNode__Icon stepNode__clickable"}>
        <CubesIcon />
      </div>
      {!(data.connectorType === "END") && (
        <Handle type="source" position={Position.Right} id="b" style={{ borderRadius: 0 }} isConnectable={false} />
      )}
